...
This project will explore US airline survey data. The survey includes information about customers that is uncontrollable by the airline. For example gender, age, flight distance or purpose of travel. It also includes the satisfaction level of variables that are within the airline's control. For example cleanliness, gate location or online services. Most importantly the survey includes how satisfied each customer is with their flight. The models shown throughout this project will highlight how much a customer's satisfaction relies on variables outside of the airline's control.

## Usefulness of this model

It can be assumed that the purpose of the surveys handed out to customers was to make improvements and gauge how well the airline is doing in different fields. Finding how certain variables hold more impact than others in the overall satisfaction of a customer can help US airlines focus on certain areas to improve. The findings of this model can also dictate how high of a goal to set for a satisfied to neutral or dissatisfied ratio as complete satisfaction cannot be expected from all customers. While the models might not show that the uncontrollable variables have as high of an impact on overall satisfaction as the controllable variables, it may be noted that improvements can be made that could alter the results of the surveys.
...
The models shown in this project show how well a prediction can be made on variables outside of the airline's control.

The accuracy levels for all of the models were surprisingly high for all of the models. Overall the k nearest neighbors model returned the highest mean with the lowest standard deviation. Thus, the highest prediction accuracy can be found using this model. This landed at about 0.824 accuracy when predicting observations in the testing set.

It would be interesting to look into this data further to see how the accuracy levels change for the same models fit to the controlled variable data sets. This would require more time to fit the models again. I would predict that the models would be more accurate but not by too much.

The information found by this model could be looked into deeper to potentially indicate how US airlines could cater to different groups in different ways. For example, to different ages, classes, or new customers. This could potentially be more useful than improving some of the controllable variable that would be expensive to improve by a lot.
